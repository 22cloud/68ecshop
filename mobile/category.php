...
$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'sale_number');

$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'sale_number', 'salenum'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;  
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    if ($max > 0)
    {
        $where .= " AND g.shop_price <= $max ";
			    ...
			    /* 获得商品列表 */
    $sql = 'SELECT (SELECT count(*) FROM ecs_order_info AS oi LEFT JOIN ecs_users AS u ON oi.user_id = u.user_id, ecs_order_goods AS og WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = g.goods_id) AS sale_number,g.goods_id, g.goods_name, g.goods_name_style, g.click_count, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
