...
if ($_REQUEST['act'] == 'list')
{
    ...
    $default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'special_price' : 'goods_saled_number');

    $sort    = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'special_price', 'goods_saled_number'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
    ...
    if (!$smarty->is_cached('special.dwt', $cache_id))
    {
        /* 如果页面没有被缓存则重新获取页面的内容 */
        ...
        $position = assign_ur_here('special');
        ...
        $count = get_special_goods_count($children, $integral_min, $integral_max);
        ...
        assign_pager('special',            $cat_id, $count, $size, $sort, $order, $page, '', '', $integral_min, $integral_max, $display); // 分页
        assign_dynamic('exchange_list'); // 动态内容
    }
    
    $smarty->assign('feed_url',         ($_CFG['rewrite'] == 1) ? "feed-typespeical.xml" : 'feed.php?type=special'); // RSS URL
...
function special_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $where = "sg.is_special = 1 AND g.is_delete = 0 AND ".
             "($children OR " . get_extension_goods($children) . ')';

    if ($min > 0)
    {
        $where .= " AND sg.special_price >= $min ";
    }

    if ($max > 0)
    {
        $where .= " AND sg.special_price <= $max ";
    }

    /* 获得商品列表 */
    $sql = 'SELECT (SELECT count(*) FROM ecs_order_info AS oi LEFT JOIN ecs_users AS u ON oi.user_id = u.user_id, ecs_order_goods AS og WHERE oi.order_id = og.order_id AND ' . time() . ' - oi.add_time < 2592000 AND og.goods_id = g.goods_id) AS sale_number,g.goods_id, g.goods_name, g.goods_name_style, sg.special_price,' .
                'g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('special_goods') . ' AS sg, ' .$GLOBALS['ecs']->table('goods') . ' AS g ' .
            "WHERE sg.goods_id = g.goods_id AND $where $ext ORDER BY $sort $order";
...
{
    $where  = "sg.is_special = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('special_goods') . ' AS sg, ' .
           $GLOBALS['ecs']->table('goods') . " AS g WHERE sg.goods_id = g.goods_id AND $where $ext";
