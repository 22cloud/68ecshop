...
class notice
{
     ...
     * @return void
     */
    function notice($table, &$db , $id, $name)
    {
        $this->table     = $table;
        $this->db        = &$db;
        $this->id        = $id;
     ...
     * @return bool
     */
    function is_invalid_coupon($ids)
    {
        if (is_array($ids) && !empty($ids)) {
            $ids_str = implode(',',$ids);
            $sql_where = 'AND coupon_id IN ("'.$ids_str.'")';
            $run_flag = true;
        }elseif(intval($ids) > 0){
            $sql_where = 'AND coupon_id IN ("'.$ids.'")';
            $run_flag = true;
        }else{
            $run_flag = false;
        }
        ...
        if ($run_flag) {
            $sql = 'SELECT COUNT(*) FROM ecs_users_coupons WHERE is_invalid = 0 '.$sql_where.' limit 1';
            $has_uninvalid = $this->db->getOne($sql);
        }else{
            $has_uninvalid = true;
        }

        return $has_uninvalid ? false : true;
    }
}
