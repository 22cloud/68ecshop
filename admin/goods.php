...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        $goods_buysend_list   = get_goods_buysend($goods['goods_id']); // 买送的结果商品
    ...
    $smarty->assign('goods_buysend_list', $goods_buysend_list);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $promote_total_number = ($is_promote && !empty($_POST['promote_total_number'])) ? ($_POST['promote_total_number']) : 100;
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, promote_total_number, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$promote_start_date', '$promote_end_date', '$promote_total_number', '$goods_img', '$goods_thumb', '$original_img', ".
    ...
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "promote_total_number = '$promote_total_number', ";
...
elseif ($_REQUEST['act'] == 'add_buy_send')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    foreach ($fittings AS $val)
    {
        $sql = "INSERT INTO " . $ecs->table('goods_buysend') . " (parent_id, goods_id, goods_price, admin_id) " .
                "VALUES ('$goods_id', '$val', '$price', '$_SESSION[admin_id]')";
        $db->query($sql, 'SILENT');
    }

    $arr = get_goods_buysend($goods_id);
    $opt = array();
...
elseif ($_REQUEST['act'] == 'drop_buy_send')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    $sql = "DELETE FROM " .$ecs->table('goods_buysend') .
            " WHERE parent_id='$goods_id' AND " .db_create_in($fittings, 'goods_id');
    if ($goods_id == 0)
    {
        $sql .= " AND admin_id = '$_SESSION[admin_id]'";
